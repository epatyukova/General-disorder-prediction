...
from utils.utils import (Lamb, Lookahead, RobustL1, BCEWithLogitsLoss, EDMDataset, get_edm, CombinedScaler, count_parameters)
from utils.utils import masked_mse, masked_mae, masked_acc
...
class CrabNetLightning(L.LightningModule):
    def __init__(self, **config):
        ...
        y=np.array([train_data['form_energy_per_atom'].values,
           train_data['disorder'].values]).T # flag
        
        self.step_size = len(train_data) # flag

        self.scaler=CombinedScaler(y,scale_map=[True,False])
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred, unc = output.chunk(2, dim=1)
        pred=pred.squeeze()
        unc=unc.squeeze()
       
        loss = self.criterion_energy(pred[:,0].view(-1),
                              unc[:,0].view(-1),
                              y[:,0].view(-1))+\
               self.criterion_disorder(pred[:,1].view(-1),
                              unc[:,1].view(-1),
                              y[:,1].view(-1))             
               
        
        self.log("train_loss", loss, on_step=False, on_epoch=True, prog_bar=True, logger=True, batch_size=self.batch_size)
        # unc[:,0] = torch.exp(unc[:,0]) * self.scaler.std[0]
        # unc[:,1] = torch.exp(unc[:,1]) * self.scaler.std[1]
        pred = self.scaler.unscale(pred)
        y = self.scaler.unscale(y)
        
        mse = masked_mse(pred[:,0].view(-1),y[:,0].view(-1))
        mae = masked_mae(pred[:,0].view(-1),y[:,0].view(-1))
        acc = masked_acc(pred[:,1].view(-1),y[:,1].view(-1))
        ...
        self.log("train_acc", acc, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_loss = self.criterion_energy(pred[:,0].view(-1),
                              unc[:,0].view(-1),
                              y[:,0].view(-1))+\
               self.criterion_disorder(pred[:,1].view(-1),
                              unc[:,1].view(-1),
        ...
        self.log("val_acc", acc, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return val_loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test_acc", acc, on_step=False, on_epoch=True, prog_bar=False, logger=True, batch_size=self.batch_size)
        return 
...
if __name__=='__main__':
    ...
    path='C:/Users/patykova/Documents/Disorder-prediction-new/Multiproperty_simple/Multiproperty_simple/'
    ...
    wandb_logger = WandbLogger(project="Crabnet-multiplroperty-ener-dis", config=config, log_model="all")
    ...
    # trainer.test(ckpt_path='last',datamodule=disorder_data)
