...
def BCEWithLogitsLoss(output, log_std, target, weight=None):
    # need to add mask
    mask=~torch.isnan(torch.as_tensor(target))
    target_nonan=torch.nan_to_num(torch.as_tensor(target), nan=0.0)
...
def masked_mse(output,target):
    mask=~torch.isnan(torch.as_tensor(target))
    target_nonan=torch.nan_to_num(torch.as_tensor(target), nan=0.0)
    loss=((output-target_nonan)**2)* mask.long()
    return loss.sum()/mask.long().sum()

def masked_mae(output,target):
    mask=~torch.isnan(torch.as_tensor(target))
    target_nonan=torch.nan_to_num(torch.as_tensor(target), nan=0.0)
    loss=torch.abs(mask.long() * (output-target_nonan))
    return loss.sum()/mask.long().sum()

def masked_acc(output,target):
    mask=~torch.isnan(torch.as_tensor(target))
    y_pred = (output > 0.5).long()
    return torch.sum(torch.eq(y_pred,target).long()*mask.long())/mask.long().sum()
...
class CombinedScaler():
    def __init__(self, data, scale_map):
        self.mask=~torch.isnan(torch.as_tensor(data))
        self.data_nonan=torch.nan_to_num(torch.as_tensor(data), nan=0.0)
        
        self.mean = self.data_nonan.sum(axis=0)/self.mask.long().sum(axis=0)
        self.std = torch.sqrt(torch.sum((self.data_nonan-torch.repeat_interleave(self.mean.unsqueeze(-1), self.data_nonan.shape[0], dim=1).transpose(0,1))**2\
                                        *self.mask.long(),axis=0)/self.mask.long().sum(axis=0))
    ...
    def unscale(self, data_scaled):
        data_scaled = torch.as_tensor(data_scaled)
        data=torch.zeros_like(data_scaled)
        for i in range(len(self.scale_map)):
            if(self.scale_map[i]==True):
...
class EDMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # X = torch.as_tensor(X, dtype=data_type_torch, device=self.device)
        # y = torch.as_tensor(y, dtype=data_type_torch, device=self.device)
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    return out, [energy, stability], formula
